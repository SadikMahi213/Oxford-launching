...
async def update_kyc_status(
    ...
    if hold_amount > 0:
        if was_approved:
            # Release hold to company wallet. Resubmission fees (submission
            # #2+) go to the SEPARATE KYC Resubmission Wallet; first-KYC fees
            # keep going to total_kyc_collected exactly as before.
            is_resubmission = bool(kyc) and ((getattr(kyc, "submission_count", 1) or 1) > 1)
            user.kyc_hold = Decimal("0")
            ...
            if is_resubmission:
                company_wallet.total_kyc_resubmission_collected = (
                    (company_wallet.total_kyc_resubmission_collected or Decimal("0")) + hold_amount
                ).quantize(WALLET_PRECISION, rounding=ROUND_HALF_UP)
            else:
            ...
            wallet_txn = WalletTransaction(
                user_id=user_id,
                type=(
                    WalletTransactionType.kyc_resubmission_fee_release
                    if is_resubmission
                    else WalletTransactionType.kyc_fee_release
                ),
                wallet_type="kyc_hold",
                amount=hold_amount,
                balance_before=hold_amount,
                ...
                reference_id=kyc.id if kyc else None,
                description=(
                    "KYC Resubmission Approved - Hold Released to Resubmission Wallet"
                    if is_resubmission
                    else "KYC Approved - Hold Released"
                ),
...
async def get_realtime_stats(
    ...
    # ── Balance Area ────────────────────────────────────
    # First-submission KYC revenue only (submission #1). Resubmissions are
    # counted separately below so the two buckets never mix.
    kyc_purchases_result = await db.execute(
        select(func.coalesce(func.sum(KYC.fee_paid), 0)).where(
            KYC.payment_status == PaymentStatus.paid,
            KYC.fee_refunded == False,
            KYC.submission_count == 1,
        )
    )
    total_kyc_purchases_usd = Decimal(str(kyc_purchases_result.scalar() or 0))

    # KYC resubmission revenue: fees collected again after a rejection+refund.
    kyc_resubmission_result = await db.execute(
        select(func.coalesce(func.sum(KYC.fee_paid), 0)).where(
            KYC.payment_status == PaymentStatus.paid,
            KYC.fee_refunded == False,
            KYC.submission_count > 1,
    ...
    total_kyc_resubmission_usd = Decimal(str(kyc_resubmission_result.scalar() or 0))
    ...
    return {
        ...
        "total_kyc_resubmission_usd": format_decimal(total_kyc_resubmission_usd),
...
# ── OFA User Approval (KYC-independent) ─────────────────────────────────────
# Separate from KYC: controls login access only via users.approval_status
# Values: pending | approved | rejected — independent of admin_kyc_status / account_status / KYC
...
async def list_user_approvals(
    ...
    base_where = []
    if status != "all":
        base_where.append(User.approval_status == status)
    if search and search.strip():
        ...
        base_where.append(
            or_(
                ...
                User.email.ilike(q),
                User.user_no.ilike(q),
            )
        )
    # Exclude admins from approval queue (they are not pending)
    base_where.append(User.is_admin.is_(False))
    where_clause = and_(*base_where) if base_where else True
    total_res = await db.execute(select(func.count(User.id)).where(where_clause))
    total = total_res.scalar() or 0
    # Status counts (for tabs)
    counts = {}
    for s in ("pending", "approved", "rejected"):
        cr = await db.execute(select(func.count(User.id)).where(User.approval_status == s, User.is_admin.is_(False)))
        counts[s] = cr.scalar() or 0
    result = await db.execute(
        select(User).where(where_clause).order_by(User.created_at.desc()).offset(offset).limit(limit)
    )
    users = result.scalars().all()
    return {
        "total": total,
        ...
        "users": [
            {
                ...
                "kyc_status": None,
                "rejection_message": getattr(u, "rejection_message", None),
            }
            for u in users
        ],
    }
...
@router.patch("/user-approvals/{user_id}/approve")
async def approve_user_approval(
    user_id: int,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_admin: User = Depends(get_current_admin_user),
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    if user.is_admin:
        raise HTTPException(status_code=400, detail="Admin users cannot be approved via this flow")
    if (getattr(user, "approval_status", "pending") or "pending").lower() == "approved":
        return {"message": "User already approved", "approval_status": "approved"}
    ...
    await notify_admin(
        ...
        message=f"User {user.full_name} ({user.email}) approved for login by admin",
        user_id=user.id,
        request=request,
    )
...
@router.patch("/user-approvals/{user_id}/reject")
async def reject_user_approval(
    user_id: int,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_admin: User = Depends(get_current_admin_user),
    rejection_message: str = Body(default="", embed=True),
):
    res = await db.execute(select(User).where(User.id == user_id).with_for_update())
    user = res.scalar_one_or_none()
    if not user:
    ...
    if (getattr(user, "approval_status", "pending") or "pending").lower() == "rejected":
        return {"message": "User already rejected", "approval_status": "rejected"}
    user.approval_status = "rejected"
    user.rejection_message = rejection_message or None
    await db.commit()
    await db.refresh(user)
    await notify_admin(
        db=db,
        type="user_approval_rejected",
        message=f"User {user.full_name} ({user.email}) rejected by admin",
        user_id=user.id,
        request=request,
    )
