...
export default function UserApproval() {
  ...
  const handleReject = async (u) => {
    ...
    setActionId(u.id);
    setMsg(""); setError("");
    const message = window.prompt("Enter custom rejection message (optional):", "");
    if (message === null) { setActionId(null); return; }
    setRejectionMessage(message.trim());
    try {
      await rejectUserApproval(token, u.id, message.trim());
      setMsg(`Rejected ${u.email}`);
      fetchData();
    } catch (e) {
      setError(e.response?.data?.detail || "Reject failed");
    } finally { setActionId(null); setRejectionMessage(""); }
  };
                ...
                <tr key={u.id} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {u.approval_status === "rejected" && (
                        <span className="text-xs text-red-400">{u.rejection_message || "No message"}</span>
                      )}
                    </div>
                  </td>
                </tr>
