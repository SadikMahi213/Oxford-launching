...
@section('title','Funding Allocation Form')
@section('content')
<a href="{{ route('admin.funding.index') }}" class="text-sm border px-3 py-1.5 rounded-full bg-white">← Back</a>
<div class="mt-4 bg-white border border-[#e8e0d0] rounded-2xl p-6 max-w-[720px]">
  <h2 class="font-display font-bold text-lg">{{ isset($item) ? 'Edit' : 'Create' }} Funding Allocation</h2>
  <form method="POST" action="{{ isset($item) ? route('admin.funding.update', $item->id) : route('admin.funding.store') }}" enctype="multipart/form-data" class="mt-4 space-y-4">
    @csrf
    @if(isset($item)) @method('PUT') @endif
      ...
      <div><label class="text-xs font-medium">%</label><input type="number" step="any" name="percentage" value="{{ old("percentage", $item->percentage ?? "") }}" class="mt-1 w-full border rounded-lg px-3 py-2 text-sm"></div>
      <div><label class="text-xs font-medium">Amount BDT</label><input type="number" step="any" name="amount" value="{{ old("amount", $item->amount ?? "") }}" class="mt-1 w-full border rounded-lg px-3 py-2 text-sm"></div>
      ...
      <div><label class="text-xs font-medium">Color</label><input type="text" name="color" value="{{ old("color", $item->color ?? "") }}" class="mt-1 w-full border rounded-lg px-3 py-2 text-sm"></div>
