...
@section('title','Investment Term Form')
@section('content')
<a href="{{ route('admin.investment-terms.index') }}" class="text-sm border px-3 py-1.5 rounded-full bg-white">← Back</a>
<div class="mt-4 bg-white border border-[#e8e0d0] rounded-2xl p-6 max-w-[720px]">
  <h2 class="font-display font-bold text-lg">{{ isset($item) ? 'Edit' : 'Create' }} Investment Term</h2>
  <form method="POST" action="{{ isset($item) ? route('admin.investment-terms.update', $item->id) : route('admin.investment-terms.store') }}" enctype="multipart/form-data" class="mt-4 space-y-4">
    @csrf
