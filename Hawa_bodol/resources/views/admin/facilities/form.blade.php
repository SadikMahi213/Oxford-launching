...
@section('content')
<a href="{{ route('admin.facilities.index') }}" class="text-sm border px-3 py-1.5 rounded-full bg-white">← Back</a>
<div class="mt-4 bg-white border border-[#e8e0d0] rounded-2xl p-6 max-w-[720px]">
  <h2 class="font-display font-bold text-lg">{{ isset($item) ? 'Edit' : 'Create' }} Facility</h2>
  <form method="POST" action="{{ isset($item) ? route('admin.facilities.update', $item->id) : route('admin.facilities.store') }}" enctype="multipart/form-data" class="mt-4 space-y-4">
    @csrf
