...
<div class="flex justify-between items-center">
  <h1 class="font-display font-bold text-xl">FAQ</h1>
  <a href="{{ route('admin.faqs.create') }}" class="bg-[#1a3a2a] text-white px-5 py-2 rounded-full text-sm">+ Add New</a>
</div>
        ...
        <tr class="border-t">
          ...
          <td class="p-3">
            <div class="flex gap-1 justify-center">
              <a href="{{ route('admin.faqs.edit', $it->id) }}" class="bg-white border px-3 py-1 rounded-full text-xs">Edit</a>
              <form method="POST" action="{{ route('admin.faqs.destroy', $it->id) }}" onsubmit="return confirm('Delete?')">@csrf @method('DELETE')<button class="bg-red-50 border border-red-200 text-red-700 px-3 py-1 rounded-full text-xs">Delete</button></form>
            </div>
          </td>
        </tr>
      @empty
