...
def test_resubmit_rededucts_refunded_fee_once():
    ...
    assert kyc.fee_refunded is False, "refund flag must clear so admin accounting sees the new fee"
    assert kyc.fee_refunded_at is None
    assert kyc.submission_count == 2, "resubmission must bump the submission counter"
    assert resp["fee_deducted"] == "10"

    holds = _kyc_transactions(db, WalletTransactionType.kyc_resubmission_fee_hold)
    assert len(holds) == 1, "exactly one new resubmission hold transaction per resubmission"
    assert holds[0].status == WalletTransactionStatus.held
    assert holds[0].amount == Decimal(10)
    assert holds[0].reference_id == kyc.id
    # The resubmission fee must NOT reuse the first-KYC transaction type.
