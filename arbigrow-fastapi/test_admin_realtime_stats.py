...
- `total_kyc_purchases_usd` uses stored `fee_paid` and excludes refunded fees
  (no live kyc_fee config x count multiplication), split by submission_count
  so first-KYC revenue never mixes with `total_kyc_resubmission_usd`.
...
class _FakeDashboardDB:
    ...
    def __init__(self):
        ...
        self.kyc_resubmission = Decimal("0")   # resubmission fees (submission #2+), default none
    ...
    async def execute(self, stmt):
        ...
        if "kyc_verifications" in t:
            ...
            assert "submission_count" in t, "KYC revenue must split first vs resubmission"
            if "> 1" in t:
                return _Row(self.kyc_resubmission)
...
def test_kyc_first_vs_resubmission_split():
    # First-submission revenue and resubmission revenue must never mix.
    db = _FakeDashboardDB()
    db.kyc_resubmission = Decimal("30")  # 3 resubmitted x 10 fee
    r = _fetch(db)
    assert r["total_kyc_purchases_usd"] == "90.00000000000000"
    assert r["total_kyc_resubmission_usd"] == "30.00000000000000"


def test_kyc_resubmission_zero_by_default():
    db = _FakeDashboardDB()
    r = _fetch(db)
    assert r["total_kyc_resubmission_usd"] == "0.00000000000000"
