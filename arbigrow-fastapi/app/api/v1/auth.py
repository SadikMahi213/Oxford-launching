...
PENDING_LOGIN_MESSAGE = "Your account is pending approval by the OFA Management. Please wait for admin approval before logging in."
REJECTED_LOGIN_MESSAGE = "Your account has been rejected by the OFA Management. Please contact the OFA Support Team for further assistance."


def _get_approval_status(user: User) -> str:
    return (getattr(user, "approval_status", None) or APPROVAL_STATUS_PENDING).strip().lower()


def _is_pending_approval(user: User) -> bool:
    return _get_approval_status(user) == APPROVAL_STATUS_PENDING


def _is_rejected(user: User) -> bool:
    return _get_approval_status(user) == APPROVAL_STATUS_REJECTED


def _is_approved(user: User) -> bool:
    return _get_approval_status(user) == APPROVAL_STATUS_APPROVED
...
async def login(request: Request, response: Response, user_data: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    if _is_rejected(user):
        raise HTTPException(
            status_code=403,
            detail={"code": "USER_REJECTED", "message": REJECTED_LOGIN_MESSAGE},
        )
...
async def refresh_session(
    ...
    # Enforce User Approval on refresh (KYC-independent)
    if not getattr(user, "is_admin", False):
        _ap = (getattr(user, "approval_status", None) or "pending").strip().lower()
        if _ap == "rejected":
            raise HTTPException(
                status_code=403,
                detail={
