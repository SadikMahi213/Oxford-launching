...
<<<<<<< HEAD
        ...
        # Re-deduct fee if previous was refunded. This is a KYC RESUBMISSION:
        # it gets its own resubmission fee transaction (routed to the separate
        # admin KYC Resubmission Wallet on approval) and clears the refunded
        # flags so admin accounting picks up the new fee. A repeat call finds
        # payment_status == paid and deducts nothing (idempotent retry).
        fee_deducted = "0"
        if existing_kyc.payment_status == PaymentStatus.refunded and existing_kyc.fee_paid > 0:
            ...
            existing_kyc.submission_count = (getattr(existing_kyc, "submission_count", 1) or 1) + 1
            fee_deducted = str(fee)

            wallet_txn = WalletTransaction(
                user_id=user_id,
                type=WalletTransactionType.kyc_resubmission_fee_hold,
                ...
                description="KYC Resubmission Fee Placed on Hold",
