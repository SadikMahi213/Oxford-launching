  ...
  const renderSummaryCard = (c, isSoon) => {
    ...
    if (isSoon) {
      return (
        <div
          key={c.key}
          className="group relative h-full rounded-2xl bg-gradient-to-br from-white/10 to-white/[0.02] p-px opacity-70 transition-all duration-300 hover:-translate-y-1 hover:opacity-100 hover:shadow-lg hover:shadow-white/5"
        >
          <div className="relative flex h-full items-center gap-3 overflow-hidden rounded-[15px] border border-dashed border-white/10 bg-[#0A122C]/95 p-4">
            ...
            <div className={`relative rounded-xl bg-gradient-to-br ${meta.iconWrap} p-px`}>
              <div className="relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-[11px] bg-gradient-to-br ${meta.iconBg}">
                <div className="pointer-events-none absolute inset-0 rounded-[11px] bg-gradient-to-br from-white/20 to-transparent" />
                <Icon size={18} strokeWidth={2.2} aria-hidden="true" className={`relative ${meta.iconText}`} />
              </div>
            </div>
            <div className="relative min-w-0 flex-1">
              <div className="truncate text-xs font-medium text-gray-400">{label}</div>
              <div className="mt-1.5 inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-gray-500">
    ...
    return (
      <div
        key={c.key}
        className={`group relative h-full rounded-2xl bg-gradient-to-br ${meta.gradient} p-px shadow-lg shadow-black/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${meta.glowShadow}`}
      >
        <div className="relative flex h-full flex-col overflow-hidden rounded-[15px] bg-[#0A122C]/95 p-4">
          {/* Decorative gradient orb */}
          <div className={`pointer-events-none absolute -right-7 -top-10 h-28 w-28 rounded-full bg-gradient-to-bl ${meta.orb} opacity-30 blur-2xl transition-opacity duration-300 group-hover:opacity-50`} />
          {/* Inner top highlight */}
          ...
          <div className="relative flex items-start justify-between gap-2">
            {/* Floating 3D icon */}
            <div className={`relative rounded-xl bg-gradient-to-br ${meta.iconWrap} p-px shadow-md`}>
              <div className="relative flex h-11 w-11 items-center justify-center overflow-hidden rounded-[11px] bg-gradient-to-br ${meta.iconBg}">
                <div className="pointer-events-none absolute inset-0 rounded-[11px] bg-gradient-to-br from-white/20 to-transparent" />
                <Icon size={19} strokeWidth={2.2} aria-hidden="true" className={`relative ${meta.iconText} drop-shadow-[0_2px_3px_rgba(0,0,0,0.45)]`} />
              </div>
            </div>
            <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${meta.currencyChip}`}>
              <StreamIcon size={11} aria-hidden="true" />
              {t(`ledger.stream.${streamName}`, streamName)}
            </span>
          </div>

          <div className="relative mt-3 text-xs font-medium leading-snug text-gray-400">{label}</div>

          <div className="relative mt-1 flex items-baseline gap-1.5 text-lg font-bold text-white sm:text-xl">
            <span className="truncate">{formatAmount(c.amount)}</span>
            <span className="text-[11px] font-semibold text-gray-400">{c.currency}</span>
          </div>
  ...
  return (
    <div className="animate-[fadeIn_0.4s_ease] rounded-xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] backdrop-blur-xl border border-white/10 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 border-b border-white/10">
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 auto-rows-fr">
            {activeCards.map((c) => renderSummaryCard(c, false))}
          </div>
          {soonCards.length > 0 && (
            <div className="mt-4">
              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{t("ledger.comingSoon", "Coming Soon")}</h4>
              <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 auto-rows-fr">
                {soonCards.map((c) => renderSummaryCard(c, true))}
              </div>
            </div>
          )}
        </div>
