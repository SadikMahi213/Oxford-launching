...
const OverviewPage = ({ setActivePage }) => {
  ...
  // STARD Mining is currently Soon: no mining status fetch, no countdown,
  // no start/claim actions, no mining transactions.
  ...
  // STARD Mining is currently Soon: start/claim handlers removed so no
  // mining API calls, records, transactions, polling, or background jobs run.
  ...
  return (
    <div className="min-h-screen bg-[#0a0b1e] px-4 pb-28 space-y-4">
      ...
      {/* ===== OFA Token Mining Wallet (currently Soon) ===== */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        {/* STARD Mining is not active: Soon placeholder only. No mining
            status fetch, no start/claim actions, no mining records or
            transactions, no polling. */}
        <div className="relative p-6 rounded-2xl bg-white/[0.03] backdrop-blur-md border border-white/[0.06] overflow-hidden mb-4">
          <div className="flex flex-col md:flex-row items-start gap-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-400 mt-2">{t("userDashboard.comingSoon")}</p>
            </div>
          </div>
        </div>
