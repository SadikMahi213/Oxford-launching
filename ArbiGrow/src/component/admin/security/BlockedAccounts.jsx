...
export default function BlockedAccounts() {
  ...
  const handleSaveSettings = async () => {
    ...
    const lockoutMins = Number(settings.login_lockout_minutes) || 30;
    try {
      const result = await updateSecuritySettings(token, {
        login_max_attempts: maxAttempts,
        login_lockout_minutes: lockoutMins,
      });
      setSettingsMsg("Security settings updated successfully");
      setSettings({ login_max_attempts: result.login_max_attempts ?? maxAttempts, login_lockout_minutes: result.login_lockout_minutes ?? lockoutMins });
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const v = e.target.value;
                    if (v === "") { setSettings(s => ({ ...s, login_lockout_minutes: "" })); return; }
                    const n = parseInt(v, 10);
                    if (!isNaN(n)) setSettings(s => ({ ...s, login_lockout_minutes: Math.max(1, Math.min(1440, n)) }));
                  }}
                  onBlur={(e) => { if (e.target.value === "") setSettings(s => ({ ...s, login_lockout_minutes: 30 })); }}
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none transition-colors focus:border-cyan-500/60"
                />
                <p className="mt-1 text-xs text-gray-500">Duration in minutes before a blocked account is automatically unblocked (1–1440)</p>
              </div>
      ...
      {activeTab === "blocked" && (
        <>
          ...
          {loading ? (
            ...
            <>
              {/* Desktop table */}
              <div className="hidden md:block rounded-2xl bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl border border-white/10 overflow-hidden">
                <div className="responsive-table-wrapper">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/10 text-xs text-gray-400 uppercase">
                    ...
                    <tbody>
                      {users.map((u) => (
                        <tr key={u.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className="inline-block px-2 py-0.5 rounded-lg text-xs font-medium bg-red-500/10 text-red-400">Blocked</span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`inline-block px-2 py-0.5 rounded-lg text-xs font-medium ${
                              ...
                              "bg-gray-500/10 text-gray-400"
                            }`}>{u.failed_attempts}</span>
                          </td>
                          <td className="px-4 py-3 text-xs text-gray-400">
                            {u.blocked_at ? new Date(u.blocked_at).toLocaleString() : "-"}
                          </td>
                          <td className="px-4 py-3 text-sm font-mono text-gray-400">{u.last_login_ip || "-"}</td>
                          <td className="px-4 py-3 text-xs text-gray-400 max-w-[200px] truncate">{u.blocked_reason || "-"}</td>
              ...
              <div className="md:hidden space-y-3">
                {users.map((u) => (
                  <div key={u.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="shrink-0 inline-block px-2 py-0.5 rounded-lg text-[11px] font-medium bg-red-500/10 text-red-400">Blocked</span>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-400">
                      <span>Failed: <span className={`font-medium ${
                        u.failed_attempts >= 10 ? "text-red-400" :
                        u.failed_attempts >= 5 ? "text-yellow-400" :
                        "text-gray-300"
                      }`}>{u.failed_attempts}</span></span>
                      {u.last_login_ip && <span className="font-mono">{u.last_login_ip}</span>}
                      {u.blocked_at && <span>{new Date(u.blocked_at).toLocaleString()}</span>}
                    </div>
                    {u.blocked_reason && (
                      <p className="text-xs text-gray-500 leading-relaxed line-clamp-2">{u.blocked_reason}</p>
                    )}
                    <button
                      onClick={() => handleUnblock(u.id)}
                      className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-green-500/15 border border-green-500/30 text-green-400 text-sm font-medium hover:bg-green-500/25 active:scale-[0.98] transition-all"
                    >
