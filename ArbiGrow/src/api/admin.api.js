...
export const getUserApprovals = async (token, { page = 1, limit = 50, search = "", status = "all" } = {}) => {
  const params = new URLSearchParams();
  params.append("page", page);
  params.append("limit", limit);
  if (search) params.append("search", search);
  if (status) params.append("status", status);
  const res = await api.get(`v1/admin/user-approvals?${params.toString()}`, authHeaders(token));
  return res.data || {};
};

export const approveUserApproval = async (token, userId) => {
  const res = await api.patch(`v1/admin/user-approvals/${userId}/approve`, {}, authHeaders(token));
  return res.data || {};
};

export const rejectUserApproval = async (token, userId) => {
  const res = await api.patch(`v1/admin/user-approvals/${userId}/reject`, {}, authHeaders(token));
  return res.data || {};
};
