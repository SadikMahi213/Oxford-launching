...
import {
  getReferralNetwork, getActiveAnnouncement,
  refreshUserStore,
...
export function UserDashboard() {
  ...
  // Transaction history is currently Soon: no fetch effect here on purpose.
  // No transaction-history API calls, no polling, no pagination/filtering.
  ...
  // Placeholder for temporarily inactive features: shows "Soon"
  // immediately without any API calls, polling, or data loading.
  const SoonView = ({ title }) => (
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="text-center">
        ...
        <p className="text-gray-400">{t("userDashboard.comingSoon")}</p>
      </div>
    </div>
  );
  ...
  const renderPageContent = () => {
    ...
    // Transaction history is currently Soon: render placeholder immediately,
    // no transaction-history API calls, no polling, no pagination/filtering.
    if (activePage === "transactions") {
      return <SoonView title={t("userDashboard.sidebar.transactions")} />;
    }
    ...
    // Marketplace is not active: Soon placeholder, no product/data/API loading.
    if (activePage === "marketplace") {
      return <SoonView title={t("userDashboard.sidebar.marketplace")} />;
    }

    // Seller is not active: Soon placeholder, no seller dashboard/API loading.
    if (activePage === "seller") {
      return <SoonView title={t("userDashboard.sidebar.seller")} />;
    }
